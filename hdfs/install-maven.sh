...
# create profile
check_var_exist $JAVA_HOME $(echo "export JAVA_HOME=/usr/lib/jvm/$JAVA_DIR" >>$MAVEN_PROFILE)
check_var_exist $M2_HOME \
  $(
    echo "export M2_HOME=${M2_HOME}" >>$MAVEN_PROFILE
    echo "export JAVA_HOME=/usr/lib/jvm/$JAVA_DIR" >>$MAVEN_PROFILE
    echo "export PATH=${M2_HOME}/bin:${PATH}" >>$MAVEN_PROFILE
  )
